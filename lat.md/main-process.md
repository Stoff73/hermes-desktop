...
[[src/main/config.ts#ensureLocalApiServerKey]] runs from [[src/main/hermes.ts#buildGatewayEnv]], immediately after `ensureApiServerConfig`, and writes a fresh `randomBytes(24)` hex key to the profile `.env` when the current value is missing or a placeholder. [[src/main/config.ts#isUsableApiServerKey]] is the single definition of "usable" (at least 16 chars, not `changeme`/`your-api-key`/similar); [[src/main/ssh-remote.ts]] imports and re-exports it so the local and SSH bars can never drift.
