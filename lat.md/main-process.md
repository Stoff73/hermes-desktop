...
## Local api_server key provisioning

`API_SERVER_KEY` is a loopback secret shared between the desktop and the Hermes gateway on the same machine. It is not a provider credential and is not tied to any account, so the app mints it rather than asking the user for one.

[[src/main/config.ts#ensureLocalApiServerKey]] runs from [[src/main/hermes.ts#buildGatewayEnv]], immediately after `ensureApiServerConfig`, and writes a fresh `randomBytes(24)` hex key to the profile `.env` when the current value is missing or a placeholder. "Current" means the profile's **own** sources (its `.env`, vault, or config.yaml) — the same ones the `EMPTY_API_SERVER_KEY` health check reads — never the default profile's key that `getApiServerKey` falls back to. Judging the fallback let a new agent chat fine (its gateway inherited the default key) while the Diagnose banner insisted no key was set. [[src/main/config.ts#isUsableApiServerKey]] is the single definition of "usable" (at least 16 chars, not `changeme`/`your-api-key`/similar); [[src/main/ssh-remote.ts]] imports and re-exports it so the local and SSH bars can never drift.

This closes a long-standing asymmetry. SSH mode always provisioned the key ([[main-process#SSH api_server provisioning]]), while local mode only ever *read* it — so a local user with no key got a config-health warning telling them to invent a secret by hand, and the surrounding copy called it an "API key", which reads as a provider credential. The generator behind Settings → Connection ([[src/renderer/src/components/settings/ConnectionPane.tsx]]) and the Diagnose banner's modal both still exist for repair; they are now a fallback rather than the only path.

### Copy names it a local key

Every user-facing string for `API_SERVER_KEY` says *local gateway key* and states that nothing leaves the machine.
