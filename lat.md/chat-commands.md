...
The tool expects an answer per `qid`: a plain-text reply to a batch is treated as cancel-all, and the gateway's `clarify.respond` locks one answer per `question_id`, replying with the `remaining` qids and releasing the tool when none are left. [[src/shared/clarify.ts#parseClarifyQuestions]] reads the batch and [[src/shared/clarify.ts#formatClarifyQuestion]] renders one question as "Question N of M" with numbered choices. On the dashboard transport, [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#appendClarifyQuestion]] shows the first question as a local-only bubble; [[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#useDashboardChatTransport]] sends each reply with that question's id and appends the next open question until the gateway reports none remain, at which point the composer is a normal prompt again.
