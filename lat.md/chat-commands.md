...
The gateway's clarify tool can ask several questions in one `clarify.request` (payload `questions: [{qid, question, choices, multi_select}]`), and the desktop answers them one composer reply at a time.

The tool expects an answer per `qid`: a plain-text reply to a batch is treated as cancel-all, and the gateway's `clarify.respond` locks one answer per `question_id`, replying with the `remaining` qids and releasing the tool when none are left. [[src/shared/clarify.ts#parseClarifyQuestions]] reads the batch and [[src/shared/clarify.ts#formatClarifyQuestion]] renders one question as "Question N of M" with numbered choices. On the dashboard transport, [[src/renderer/src/screens/Chat/dashboardEventAdapter.ts#appendClarifyQuestion]] shows the first question as a local-only bubble; [[src/renderer/src/screens/Chat/hooks/useDashboardChatTransport.ts#useDashboardChatTransport]] sends each reply with that question's id, appends the next open question, and puts the chat back into the awaiting-answer state (active turn cleared, not loading) exactly as `clarify.request` left it — until the gateway reports none remain, at which point the composer is a normal prompt again. Restoring that state matters: `handleSend` marks every reply as a running turn, and no completion arrives mid-clarify, so without it the second answer was shunted into the busy queue and the batch stalled until the user pressed stop (which then drained the answer as a fresh prompt).

Before this the adapter looked only for a single `question`, found none in a batch payload, and silently dropped the request: the chat showed the agent's thoughts and tool calls and then nothing, while the agent waited on a prompt nobody could see (and any reply would have cancelled every question). The legacy main-process path ([[src/main/hermes.ts]] `clarify.request`) still shows a batch as one free-text card; local mode does not use it once the dashboard is up.
